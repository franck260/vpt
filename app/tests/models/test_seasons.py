...
class TestSeason(ModelTestCase):
    ...
    def test_get(self):
        ...
        self.assertEqual(season_1.start_year, 2009)
        self.assertEqual(season_1.end_year, 2010)
        self.assertEqual(season_2.start_year, 2010)
        self.assertEqual(season_2.end_year, 2011)
    ...
    def test_players(self):
        ...
        self.assertEqual(len(season_1.players), 3)
        self.assertIn(franck_p, season_1.players)
        self.assertIn(jo, season_1.players)
        self.assertIn(nico, season_1.players)

        self.assertListEqual(season_2.players, [])
    ...
    def test_results(self):
        ...
        self.assertListEqual(results_season_2, [])

    def test_raw_results(self):
        season_1 = config.orm.query(Season).filter(Season.start_year == 2009).one() #@UndefinedVariable
        season_2 = config.orm.query(Season).filter(Season.start_year == 2010).one() #@UndefinedVariable

        self.assertListEqual([r.tournament.id for r in season_1.raw_results], [1, 1, 1, 2, 2])
        self.assertListEqual(season_2.raw_results, [])